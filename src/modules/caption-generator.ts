...
export class CaptionGenerator {
  ...
  async generate(
    ...
    const wordsPerGroup = jobOptions.captions.maxWordsPerGroup;
    ...
    const groups = this.groupWords(framed, wordsPerGroup);
    ...
    const { width, height } = getAspectDimensions(jobOptions.output.aspectPreset);
    ...
    const inputProps: CaptionOverlayProps = {
      ...
      fps: FPS,
      durationInFrames,
      captions: jobOptions.captions,
    };

    ensureDir(dirname(outputPath));

    log.info(`Rendering caption overlay (${groups.length} groups, ${durationInFrames} frames, ${width}x${height})...`);
  ...
  private groupWords(words: CaptionWord[], wordsPerGroup: number): CaptionGroup[] {
